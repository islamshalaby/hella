...
class StatsController extends AdminController{
    public function show() {
        ...
        $data['canceled_orders'] = Order::where('status', 3)->count();
        $data['delivered_orders'] = Order::where('status', 3)->count();
        $data['delivered_orders_cost'] = Order::where('status', 2)->sum('total_price');
        $data['canceled_orders_cost'] = Order::where('status', 3)->sum('total_price');
        $data['in_progress_orders_cost'] = Order::where('status', 1)->sum('total_price');
        $data['total_orders_cost'] = Order::sum('total_price');
        $data['cash_orders_cost'] = Order::where('payment_method', 3)->where('status', 2)->sum('total_price');
        $data['key_net_orders_cost'] = Order::where('payment_method', 1)->where('status', 2)->sum('total_price');
        ...
        $data['canceled_orders_today_count'] = Order::where('status', 3)->whereDate('created_at', Carbon::today())->count();
        $data['delivered_orders_today_count'] = Order::where('status', 2)->whereDate('created_at', Carbon::today())->count();
        $data['today_delivered_orders_cost'] = Order::where('status', 2)->whereDate('created_at', Carbon::today())->sum('total_price');
        $data['today_canceled_orders_cost'] = Order::where('status', 3)->whereDate('created_at', Carbon::today())->sum('total_price');
        $data['today_in_progress_orders_cost'] = Order::where('status', 1)->whereDate('created_at', Carbon::today())->sum('total_price');
        $data['today_cash_cost'] = Order::where('payment_method', 3)->where('status', 2)->whereDate('created_at', Carbon::today())->sum('total_price');
        $data['today_key_net_cost'] = Order::where('payment_method', 1)->where('status', 2)->whereDate('created_at', Carbon::today())->sum('total_price');
